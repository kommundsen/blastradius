...
The **reference** loads on its own when architecture comes up — which is
exactly why it must not start asking questions. The **workflows** you invoke
deliberately, so they can:

- **model** — builds or extends the model by interviewing you first: scope, how
  much detail, whether to attach ADRs and other docs, whether components should
  point at their source for code-level detail, and whether there is a
  deployment story worth drawing. On a repository that already has code it
  surveys first and brings you a proposal to correct; on an empty one it has
  nothing to read, so it asks properly.
- **sync** — finds where the model has drifted from the code since a given
  commit and proposes the fixes.
- **review** — judges the model against the repository and reports, without
  changing anything.

Each agent gets these in its own format, because each one looks somewhere
different:

| Agent | Workflows | Survey subagent |
| --- | --- | --- |
| Claude Code | `/blastradius:model`, `:sync`, `:review` | `blastradius-surveyor` |
| GitHub Copilot | `/blastradius-model` and friends, from `.github/prompts/` | `blastradius-surveyor` |
| Cursor | skills, invoked by name | — |
| Codex | skills, invoked by name | — |

Cursor and Codex both read `.agents/skills/`, so one set of files serves both.
